...
struct HomeView: View {
    ...
    var body: some View {
        VStack(spacing: 20) {
            ...
            Text("The time that leads to mastery is depdent on the intensiffjdskd of the focus").font(.title3).fontWeight(.light).foregroundColor(.secondary).multilineTextAlignment(.center).padding()
            ...
            DispatchQueue.main.asyncAfter(deadline: .now() + 1) { isAnimating = true }
        })
    }
}
